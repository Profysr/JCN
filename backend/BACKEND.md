...
| GET | `/api/workspaces/{ws}/hr/holidays/suggestions/?country=XX&year=YYYY` | Admin (`hr.manage_leave`, read scope): proxies [Nager.Date](https://date.nager.at) (free, keyless) for that country/year — `{ country, year, results: [{name, date, is_recurring}] }`. `is_recurring` mirrors Nager's `fixed` flag (same month/day every year). Degrades to `results: []` on any upstream/network failure — never errors the request. Powers the "Import holidays" checklist in `LeavePage.jsx`. |
| POST | `/api/workspaces/{ws}/hr/holidays/bulk/` | Bulk-create holidays (admin only). Body: `{ holidays: [{name, date, is_recurring?, location?}, …] }` (max 100). Dedupes against the workspace+date+name unique constraint via `get_or_create` (no 400 on overlap) — response: `{ created: [...], skipped: N }`. Backs both the Nager.Date import checklist and manually-typed one-off rows added in the same modal. |
