...
| `OnboardingState` | `workspace` (O2O), `wizard_completed`, `module_ui_state` (JSONField `{"projects": {"dismissed": ["uuid1"], "minimized": [], "welcomed": ["uuid1"]}}`) | Per-user, per-module UI flags (`dismissed`/`minimized`/`welcomed`) in one field via `has_flag`/`set_flag` — add new flags to `UI_FLAGS`, no column. Checklist items computed on-the-fly from `workspaces/checklist.py` registry. GET `/onboarding/` returns `{wizard_completed, user_is_admin, checklists:{<mod>:{dismissed,minimized,welcomed,items}}}`; PATCH accepts `wizard_completed` + `module_dismiss`/`module_minimize`/`module_unminimize`/`module_welcome` (each = module key). |
...
| `OrgProfile` | `member` (O2O→WorkspaceMember), `job_title` (FK, SET_NULL), `employment_type` (full_time/part_time/contractor/intern), `employee_id`, `start_date`, `location`, `work_location_url`, `work_latitude`/`work_longitude`, `bio` | One per member; auto-created via `get_or_create` on first GET/PATCH. `HRDashboardView` reads `start_date` (joiners/anniversaries) and `employment_type` (headcount split). `work_location_url` is a Google Maps link (full or `goo.gl`/`maps.app.goo.gl` share link) pasted by the employee/HR; `OrgProfileSerializer.validate_work_location_url` parses it via `organization/geo.py::parse_maps_url` (resolves short-link redirects with `requests`, then regexes `?q=`, `/@`, `/place/` coordinate formats) and derives `work_latitude`/`work_longitude` server-side — those two fields are read-only on the API, never client-set directly. This is the reference point attendance geofencing (below) checks clock-in/out against. |
...
| `AttendancePolicy` | `workspace` (O2O), `work_start_time`, `work_end_time`, `grace_period_minutes`, `weekly_hours`, `geofence_enabled` (bool, default False), `geofence_radius_meters` (default 500) | One per workspace; auto-created on first access with sensible defaults (09:00–17:00, 15 min grace, 40 h/week). Geofencing is off by default — an admin opts in per workspace once employees have `OrgProfile.work_latitude/longitude` populated. |
| `Attendance` | `employee` (FK→WorkspaceMember), `date`, `clock_in`/`clock_out` (TimeField, nullable), `clock_in_latitude`/`longitude`, `clock_in_ip`, `clock_out_latitude`/`longitude`, `clock_out_ip`, `clock_in_outside_geofence`/`clock_out_outside_geofence` (bool), `missed_clock_out_notified_at` (nullable, dedup flag), `source` (manual/api), `notes` | unique: employee+date (one row per employee per day) — this unique index also serves employee + date-range lookups, so no separate index. `clock_out=null` means still clocked in. Lat/lng come from the browser's Geolocation API in the clock-in/out request body (null if permission denied); IPs are captured server-side from the request (`hr/views.py::_client_ip`, X-Forwarded-For first hop / REMOTE_ADDR) — never client-supplied. The `outside_geofence` flags are set by `hr/views.py::_check_geofence` (haversine distance vs `OrgProfile` location, compared to `AttendancePolicy.geofence_radius_meters`) when geofencing is enabled — **flag only, clock-in/out is never blocked**, and HR is notified (`attendance.geofence_flagged`) via `_notify_geofence_breach`. `hr.tasks.flag_missed_clock_outs` (daily, see Scheduled Tasks) flags workspace admins about yesterday's records with a `clock_in` but no `clock_out`. |
...
| `hr.tasks.flag_missed_clock_outs` | daily, 01:00 UTC | Flags workspace admins about `Attendance` rows from **yesterday** with a `clock_in` but no `clock_out` — HR follows up manually, nothing is auto-filled. Dedup via `missed_clock_out_notified_at`. |
...
| GET | `/api/workspaces/{ws}/hr/holidays/countries/` | Admin (`hr.manage_leave`, read scope): static picker list for the import country dropdown — `{ results: [{code, name}, …] }`, sourced from `hr/holiday_import.py::COMMON_COUNTRIES` (curated ~50-country subset of the `holidays` library's 140+, spanning South Asia/GCC/East Asia/Europe/Americas — not an exhaustive ISO-3166 dump). |
| GET | `/api/workspaces/{ws}/hr/holidays/suggestions/?country=XX&year=YYYY` | Admin (`hr.manage_leave`, read scope): computes that country/year's public holidays locally via the `holidays` PyPI library (140+ countries/territories, incl. Pakistan/India/Bangladesh — no network call, no API key) — `{ country, year, results: [{name, date, is_recurring}] }`. `is_recurring` is derived by checking whether the same-named holiday falls on the same month/day the following year (true for fixed civil holidays, false for lunar/Easter-based ones). Unsupported country code or any other lookup failure → clean 400 (`hr/holiday_import.py::UnsupportedCountry` / `HolidayLookupError`), never a 500. Powers the "Import holidays" checklist in `LeavePage.jsx`. |
| POST | `/api/workspaces/{ws}/hr/holidays/bulk/` | Bulk-create holidays (admin only). Body: `{ holidays: [{name, date, is_recurring?, location?}, …] }` (max 100). Dedupes against the workspace+date+name unique constraint via `get_or_create` (no 400 on overlap) — response: `{ created: [...], skipped: N }`. Backs both the country-lookup import checklist and manually-typed one-off rows added in the same modal. |
...
| POST | `/api/workspaces/{ws}/hr/attendance/clock-in/` | Clock in current user for today; errors if already clocked in. Body may include `latitude`/`longitude` (from the browser's Geolocation API) — captured on the record along with a server-derived IP; if `AttendancePolicy.geofence_enabled`, out-of-range coords set `clock_in_outside_geofence` and notify HR (flag only, never blocks). |
| POST | `/api/workspaces/{ws}/hr/attendance/clock-out/` | Clock out current user; errors if not clocked in or already out. Same optional `latitude`/`longitude` handling as clock-in, on the `clock_out_*` fields. |
...
| GET | `/api/workspaces/{ws}/hr/attendance/summary/` | Admin: per-employee summary over the range (total_hours, overtime_hours, late_count, days_present, geofence_flags); `?date_from=&date_to=` defaults to current week |
...
`AttendanceSerializer` computed fields: `status` (on_time/late/absent — compared against `AttendancePolicy.work_start_time + grace_period_minutes`), `total_hours` (float, null if no clock_out), `overtime_hours` (float, `max(0, total_hours - daily expected hours)` where daily expected hours = `work_end_time - work_start_time`; null if no `total_hours` or no policy in context).
...
**hr helpers (`hr/views.py`):** `hr/holiday_import.py::fetch_public_holidays(country_code, year)` (local computation via the `holidays` PyPI library, no network call; raises `UnsupportedCountry`/`HolidayLookupError` — used only by `HolidaySuggestionsView`); `_holiday_dates_in_range(workspace, start, end)` (expands `Holiday` rows, including recurring ones matched by month/day, into concrete dates); `_leave_days(workspace, start, end, start_day_part, end_day_part)` (Mon–Fri count, inclusive, minus workspace holidays, with a 0.5-day adjustment on the start/end date for a half-day request — replaces the old `_business_days`, which is gone); `_parse_date_window(request, default_lookback_days=31, max_span_days=366)` (bounded date-range parser used by attendance lists); `_client_ip(request)` (server-side IP capture, X-Forwarded-For first hop or REMOTE_ADDR); `_distance_meters(lat1, lng1, lat2, lng2)` (haversine, meters); `_check_geofence(workspace, member, latitude, longitude)` (True if submitted coords fall outside `AttendancePolicy.geofence_radius_meters` of the employee's `OrgProfile` location — False, i.e. not flagged, whenever geofencing is off or either side is missing coords; never raises/blocks). Leave balance create/review wrap the balance mutation in `transaction.atomic()` + `select_for_update()`. `LeaveRequest.days_requested` is computed once at creation and reused (not recomputed) at review time. All "today" logic uses `timezone.localdate()`.
...
**Not yet migrated (this pass):** `organization.OrgProfile` (`work_location_url`, `work_latitude`, `work_longitude`); `hr.AttendancePolicy` (`geofence_enabled`, `geofence_radius_meters`); `hr.Attendance` (`clock_in_latitude`/`longitude`, `clock_in_ip`, `clock_out_latitude`/`longitude`, `clock_out_ip`, `clock_in_outside_geofence`/`clock_out_outside_geofence`, `missed_clock_out_notified_at`) — attendance geofencing/IP-tracking/overtime/missed-clock-out work. `makemigrations` deliberately deferred; run it before relying on any of these fields.
