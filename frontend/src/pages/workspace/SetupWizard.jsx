...
import { useUpdateOnboarding, useWorkspaceTemplates, useApplyWorkspaceTemplate } from "@/hooks/useOnboarding";
...
/* ── Team type cards ─────────────────────────────────────────────────────── */
const TEAM_TYPES = [
  { key: "software",   label: "Software",   emoji: "💻", desc: "Agile sprints, bug tracking, releases" },
  { key: "design",     label: "Design",     emoji: "🎨", desc: "Briefs, feedback loops, deliverables" },
  { key: "marketing",  label: "Marketing",  emoji: "📢", desc: "Campaigns, content, launches" },
  { key: "operations", label: "Operations", emoji: "⚙️", desc: "Processes, SOPs, coordination" },
  { key: "education",  label: "Education",  emoji: "🎓", desc: "Curriculum, projects, assignments" },
  { key: "other",      label: "Other",      emoji: "✨", desc: "General purpose workspace" },
];
...
function EmailChipInput({ emails, onChange }) {
  ...
  return (
    <div
      className="min-h-[80px] flex flex-wrap gap-1.5 p-3 border rounded-xl bg-background focus-within:ring-1 focus-within:ring-ring cursor-text"
    ...
    >
      ...
      <input
        ...
        placeholder={emails.length === 0 ? "Enter emails, comma or space separated…" : ""}
        value={input}
...
export default function SetupWizard() {
  ...
  const handleFinish = async () => {
    // Send invites
    await Promise.allSettled(emails.map((email) => inviteMutation.mutateAsync({ email, role: inviteRole })));
    // Mark wizard complete
    await updateOnboarding.mutateAsync({ wizard_completed: true, team_type: teamType });
    ...
    <div className="min-h-screen bg-background flex flex-col">
          ...
          {STEPS.map((label, i) => (
            <div key={i} className="flex items-center gap-2">
              <div className={cn(
                ...
                : i === step ? "bg-primary/10 text-primary ring-1 ring-primary"
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-2xl animate-fade-in">
          ...
          {step === 0 && (
            <div>
              <h1 className="text-2xl font-bold text-center mb-1">What kind of team are you?</h1>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {TEAM_TYPES.map((t) => (
                  <button
                    ...
                    className={cn(
                      "p-5 rounded-xl border text-left transition-all hover:shadow-card",
                      teamType === t.key
          ...
          {step === 1 && (
            <div>
              <h1 className="text-2xl font-bold text-center mb-1">Start from a template</h1>
              <p className="text-muted-foreground text-center text-sm mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Set up everything yourself</p>
                </button>
              ...
              <p className="text-xs text-center text-muted-foreground">
                ...
                <button
                  ...
                  onClick={() => navigate(`/w/${workspaceSlug}/settings/import`)}
                >
            ...
            <div>
              ...
              <div className="flex items-center justify-between mt-8">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" onClick={handleFinish} disabled={updateOnboarding.isPending}>
                    Skip for now
                  </Button>
                  <Button
                  ...
                  >
                    {emails.length > 0
                      ? <><Send className="w-3.5 h-3.5 mr-1.5" /> Send {emails.length} invite{emails.length > 1 ? "s" : ""}</>
                      : "Finish setup"
                    }
                  </Button>
                </div>
